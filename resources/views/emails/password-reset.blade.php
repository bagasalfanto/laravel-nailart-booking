<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Reset Password — {{ config('app.name') }}</title>
</head>
<body style="margin:0; padding:0; background:#fdeae6; font-family: 'Helvetica Neue', Arial, sans-serif; color:#3a3133;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fdeae6; padding:40px 16px;">
        <tr>
            <td align="center">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px; background:#ffffff; border-radius:20px; overflow:hidden; box-shadow:0 12px 40px rgba(205,163,173,0.18);">

                    {{-- Header --}}
                    <tr>
                        <td style="padding:32px 40px 16px; text-align:center; background:linear-gradient(180deg,#fdf2f5,#ffffff);">
                            <p style="margin:0; color:#a8526b; font-size:12px; letter-spacing:3px; text-transform:uppercase; font-weight:600;">
                                {{ config('app.name', 'Nailby Bilda') }}
                            </p>
                            <h1 style="margin:16px 0 0; color:#221d1f; font-size:32px; font-weight:normal; font-family:'Georgia', serif;">
                                Reset Your Password
                            </h1>
                        </td>
                    </tr>

                    {{-- Body --}}
                    <tr>
                        <td style="padding:8px 40px 16px; color:#3a3133; font-size:15px; line-height:1.6;">
                            <p style="margin:16px 0;">Halo {{ $user->full_name ?? $user->email ?? 'User' }},</p>
                            <p style="margin:16px 0;">
                                Kami menerima permintaan untuk reset password akun kamu di {{ config('app.name') }}.
                                Klik tombol di bawah ini untuk membuat password baru:
                            </p>
                        </td>
                    </tr>

                    {{-- CTA Button --}}
                    <tr>
                        <td style="padding:8px 40px 24px; text-align:center;">
                            <a href="{{ $resetUrl }}"
                               style="display:inline-block; background:#e8a3b3; color:#ffffff; padding:14px 36px; border-radius:12px; text-decoration:none; font-size:16px; font-weight:bold; font-family:'Georgia', serif; box-shadow:0 8px 20px rgba(232,163,179,0.35);">
                                Reset Password
                            </a>
                        </td>
                    </tr>

                    {{-- Fallback link --}}
                    <tr>
                        <td style="padding:0 40px 16px; color:#64748b; font-size:12px; line-height:1.5; text-align:center;">
                            <p style="margin:0 0 8px;">Tombol tidak berfungsi? Salin link berikut ke browser:</p>
                            <p style="margin:0; word-break:break-all;">
                                <a href="{{ $resetUrl }}" style="color:#a23f66; text-decoration:underline;">{{ $resetUrl }}</a>
                            </p>
                        </td>
                    </tr>

                    {{-- Notes --}}
                    <tr>
                        <td style="padding:8px 40px 32px; color:#64748b; font-size:13px; line-height:1.6;">
                            <p style="margin:0 0 8px;">⏱ Link aktif <strong style="color:#a23f66;">{{ $expiryMinutes ?? 60 }} menit</strong>.</p>
                            <p style="margin:0;">
                                Jika kamu tidak meminta reset password, abaikan email ini.
                                Akun kamu tetap aman selama tidak mengklik link di atas.
                            </p>
                        </td>
                    </tr>

                    {{-- Footer --}}
                    <tr>
                        <td style="padding:20px 40px; background:#fdf2f5; text-align:center; color:#a8526b; font-size:12px; line-height:1.6;">
                            <p style="margin:0 0 4px;">
                                Pertanyaan? Hubungi
                                <a href="mailto:{{ $supportEmail ?? 'support@nailbyhilda.com' }}" style="color:#a23f66;">{{ $supportEmail ?? 'support@nailbyhilda.com' }}</a>
                            </p>
                            <p style="margin:0; color:#c9a3ad;">© {{ date('Y') }} {{ config('app.name') }}. Luxury Nail Studio.</p>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>
</body>
</html>
